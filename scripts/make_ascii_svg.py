"""
Convert a portrait photo into a monochrome ASCII-art SVG that "types"
itself in like a terminal, then holds.

Output: prem-ascii.svg
Usage:
    python scripts/make_ascii_svg.py [source-prepped.png] [prem-ascii.svg]
"""
...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "prem-ascii.svg")

# Ensure prepped image exists
if not os.path.exists(SRC):
    prep_script = os.path.join(HERE, "prep_photo.py")
    if os.path.exists(prep_script):
        os.system(f'"{sys.executable}" "{prep_script}"')
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">prem@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'prem@github:~$ whoami <tspan fill="{INK}">Prem Kumar Kuppili</tspan></text>')
