"""
Render "PREM" (or custom text) as an EXTRUDED 3D wordmark rasterized to ASCII,
...
TEXT = os.environ.get("WORDMARK_TEXT", "PREM")
...
def get_system_font(font_size):
    """Attempt to load a clean sans-serif/bold font across OS platforms."""
    candidates = [
        "C:\\Windows\\Fonts\\arialbd.ttf",
        "C:\\Windows\\Fonts\\arial.ttf",
        "C:\\Windows\\Fonts\\seguians.ttf",
...
def build_shell():
    ...
    if len(xs_any) > 0 and len(ys_any) > 0:
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">prem@github: ~$ ./wordmark.sh --3d</text>')
